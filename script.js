...
/* ---------------------------------------------------------
   4. GOOGLE AUTH (Firebase Auth in production)
   --------------------------------------------------------- */
function initGoogleAuth() {
  // PRODUCTION: replace with your Firebase project's OAuth client ID
  // and initialize Firebase Auth's GoogleAuthProvider instead.
  const statusEl = document.getElementById("signin-status");
  statusEl.textContent = "Demo mode — tap below to continue";
}

function handleFallbackGoogleClick() {
  // PRODUCTION: trigger firebase.auth().signInWithPopup(GoogleAuthProvider)
  completeSignIn({ name: "Demo Owner", email: "owner@biznova.app", avatar: "D" });
}
...
function buyCreditsPack() {
  launchRazorpay(299, "500 AI Credits Pack", () => {
    state.aiCreditsUsedToday = Math.max(0, state.aiCreditsUsedToday - 500);
    showToast("500 AI credits added!");
  });
}
function startPremiumUpgrade() {
  ...
  launchRazorpay(amount, `BizNova ${plan === "premium" ? "Premium" : "Pro"} Plan`, () => applyPlan(plan));
}
...
function launchRazorpay(amountRupees, description, onSuccess) {
  // PRODUCTION: replace `key` with your live Razorpay Key ID and create
  // the order server-side (Cloud Function) before opening checkout.
  ...
  const options = {
    key: "rzp_test_XXXXXXXXXXXXXX", // TODO: replace with real key
...
function renderSettings() {
  ...
  document.getElementById("profile-avatar").textContent = state.signedIn ? state.user.avatar : "U";
  ...
  const limits = { free: { hourly: 10, daily: 30 }, pro: { hourly: 50, daily: 200 }, premium: { hourly: 999, daily: 9999 } }[state.plan];
  document.getElementById("hourly-label").textContent = `${state.aiCreditsUsedHour} / ${limits.hourly} used`;
  document.getElementById("hourly-bar").style.width = Math.min(100, (state.aiCreditsUsedHour / limits.hourly) * 100) + "%";
  document.getElementById("daily-label").textContent = `${state.aiCreditsUsedToday} / ${limits.daily} used`;
  document.getElementById("daily-bar").style.width = Math.min(100, (state.aiCreditsUsedToday / limits.daily) * 100) + "%";
...
function contactSupportEmail() {
  showToast("Opening email to support@biznova.app");
}
...
function requireAiCredit(action) {
  const limits = { free: { hourly: 10, daily: 30 }, pro: { hourly: 50, daily: 200 }, premium: { hourly: 999, daily: 9999 } }[state.plan];
  if (state.aiCreditsUsedToday < limits.daily && state.aiCreditsUsedHour < limits.hourly) {
    action();
    return;
  }
  pendingAiAction = action;
  document.getElementById("unlock-ad-desc").textContent = `Free — used ${state.aiCreditsUsedToday % 3} / 3 today`;
...
function renderAiSuggestChips() {
  const chips = ["📈 Analyze my sales", "📦 Predict stock needs", "💰 Expense breakdown", "💡 Business suggestion"];
...
function sendAiMessage(preset) {
  ...
  requireAiCredit(() => {
    state.aiCreditsUsedToday++;
    state.aiCreditsUsedHour++;
    renderSettings();
...
function mockAiReply(userText, revenue, expenses, lowStock) {
  ...
  return `Based on your current numbers (revenue ₹${revenue.toLocaleString("en-IN")}, expenses ₹${expenses.toLocaleString("en-IN")}), your business looks ${revenue - expenses >= 0 ? "profitable" : "at a loss"} this period. Ask me about sales, stock, or expenses for more detail.`;
}
