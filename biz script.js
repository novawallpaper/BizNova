...
/* ---------------------------------------------------------
   2. EMPTY DATA (every new business starts at zero — no demo
      / sample / fake records of any kind, per product spec)
   --------------------------------------------------------- */
function emptyData() {
  return {
    products: [],
...
function renderDashboard() {
  ...
  const revTrend = momTrend(state.data.sales, "amount", "date");
  const expTrend = momTrend(state.data.expenses, "amount", "date");

  const kpis = [
    { label: "Revenue", value: fmtINR(revenue), icon: "fa-indian-rupee-sign", color: "#17a866", trend: revTrend.label, up: revTrend.up },
    { label: "Expenses", value: fmtINR(expenses), icon: "fa-receipt", color: "#e5484d", trend: expTrend.label, up: !expTrend.up },
  ...
  const totalRecords = state.data.products.length + state.data.customers.length + state.data.sales.length + state.data.expenses.length + state.data.invoices.length;
  const banner = document.getElementById("dashboard-empty-banner");
  if (banner) banner.style.display = totalRecords === 0 ? "flex" : "none";
...
/* Real month-over-month comparison — no invented percentages. If there's
   no data for the previous month to compare against, we say so instead
   of faking a trend. */
function momTrend(records, amountKey, dateKey) {
  const now = new Date();
  ...
  records.forEach((r) => {
    ...
    if (isNaN(d)) return;
    const k = monthKey(d);
    if (k === thisKey) thisMonth += r[amountKey] || 0;
    else if (k === prevKey) lastMonth += r[amountKey] || 0;
  });

  if (lastMonth === 0) {
    return thisMonth > 0 ? { label: "New this month", up: true } : { label: "No activity yet", up: true };
  }
  const pct = Math.round(((thisMonth - lastMonth) / lastMonth) * 100);
  return { label: `${pct >= 0 ? "+" : ""}${pct}% MoM`, up: pct >= 0 };
}
...
/* ---------------------------------------------------------
   8. REPORTS PANEL
   --------------------------------------------------------- */
function sumInRange(records, amountKey, dateKey, fromDate) {
  return records.reduce((a, r) => {
    if (!r[dateKey]) return a;
    const d = new Date(r[dateKey]);
    if (isNaN(d) || d < fromDate) return a;
    return a + (r[amountKey] || 0);
  }, 0);
}
...
function renderReportsPanel() {
  ...
  const periods = [
    { label: "Today", revenue: sumInRange(state.data.sales, "amount", "date", startOfToday), expenses: sumInRange(state.data.expenses, "amount", "date", startOfToday) },
    { label: "This Week", revenue: sumInRange(state.data.sales, "amount", "date", startOfWeek), expenses: sumInRange(state.data.expenses, "amount", "date", startOfWeek) },
    { label: "This Month", revenue: sumInRange(state.data.sales, "amount", "date", startOfMonth), expenses: sumInRange(state.data.expenses, "amount", "date", startOfMonth) },
  ];
...
function openQuickView(moduleId, id) {
  ...
  // Repurpose the top-right circular button as Delete for real business
  // records (there's no "favorite" concept for products/customers/etc).
  const favBtn = document.getElementById("modal-fav-btn");
  favBtn.classList.add("delete-mode");
  favBtn.innerHTML = '<i class="fa-solid fa-trash"></i>';
  favBtn.title = "Delete";
  favBtn.onclick = () => quickViewDelete();
...
function quickViewDelete() {
  if (!quickViewCtx) return;
  const { moduleId, id } = quickViewCtx;
  closeQuickView();
  deleteRecord(moduleId, id);
}

/* Generic delete for products/customers/sales/expenses/employees. Every
   dashboard/report/chart figure is computed live from state.data, so
   removing a record here automatically flows through everywhere —
   no separate "recalculate" step needed. */
async function deleteRecord(moduleId, id) {
  const list = state.data[moduleId];
  if (!list) return;
  const record = list.find((r) => r.id === id);
  if (!record) return;

  const label = record.name || record.title || record.customerName || "this record";
  if (!confirm(`Delete ${label}? This cannot be undone.`)) return;

  state.data[moduleId] = list.filter((r) => r.id !== id);
  await persistData();

  showToast("Deleted");
  renderDashboard();
  if (state.currentModule === moduleId) renderModuleGrid();
  renderSearch();
  renderSettings();
}
...
function openOrderScreen() {
  ...
  const productList = document.getElementById("invoice-product-list");
  if (productList) {
    productList.innerHTML = state.data.products.map((p) => `<option value="${escapeHtml(p.name)}"></option>`).join("");
  }
...
function updateInvoiceItem(idx, key, value) {
  ...
  item[key] = key === "name" ? value : Number(value || 0);

  // If the typed name matches a real product exactly, link this line to
  // it so stock can be deducted automatically when the invoice is saved,
  // and auto-fill its selling price.
  if (key === "name") {
    const match = state.data.products.find((p) => p.name.toLowerCase() === value.trim().toLowerCase());
    if (match) {
...
function renderInvoiceItems() {
  ...
  container.innerHTML = state.invoiceDraft.items
    .map(
      ...
      <input class="invoice-item-name" list="invoice-product-list" placeholder="Item name" value="${escapeHtml(it.name)}" oninput="updateInvoiceItem(${idx},'name',this.value)" />
...
async function submitOrderPayment() {
  ...
  // Deduct stock for any invoice line linked to a real product record.
  items.forEach((it) => {
    if (!it.productId) return;
    const product = state.data.products.find((p) => p.id === it.productId);
